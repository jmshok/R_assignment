...
```{r}
setwd("/Users/jmshook/Desktop/Class/cndkczy/R_assignment")
library(gtools)
library(ggplot2)
library('dplyr')
library(reshape2)
...
```
Review JS: Loop above does not end (15 minutes later)
...
Review JS: No errors in graphs, for loop took exceedingly long to finish; if this was a repeatedly used code, we would want to optimize significantly, but for single use it should be fine.